...
2. **寫入品質審核**：本回合有寫入時，逐一核對寫入後的檔案內容本身是否正確：
   （a）完整——家屬這次訊息裡每一項該記錄的資訊（數值、時間、用藥、事件）都要出現在寫入的條目中，不能漏記；
   （b）準確——檔案中的數值、日期、時間、記錄者必須與家屬訊息一致，不能抄錯或寫錯位數；
   （c）不重複——不能新增與既有條目重複的紀錄（同一事件應該是修改原條目，不是再加一筆）；
...
5. **遵守工作準則**：〈小安的工作準則〉是她被設定必須遵守的完整指引（含家屬自訂指示）。對照檢查她這回合的「行為」與「回覆」都符合準則，特別是：
   ...
   行程時間必須合理：草稿中列出的行程時間必須與 schedule 檔案內容完全一致（逐字對照，不能來自對話記憶）；「待辦」行程不應該排在已經過去的時間；家屬回報「已經發生」的事（過去式）對應的是標記完成（record_routine），把待辦行程改到過去的時間一定是錯的 → reject。
